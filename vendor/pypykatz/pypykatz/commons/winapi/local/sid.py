...
class SID:
	...
	def from_buffer(buff):
		sid = SID()
		sid.Revision = ord(buff.read(1))
		sid.SubAuthorityCount = ord(buff.read(1))
		sid.IdentifierAuthority = struct.unpack(
			'>Q', '\x00\x00' + buff.read(6)
		)
		for i in range(sid.SubAuthorityCount):
			sid.SubAuthority.append(struct.unpack('<I', buff.read(4)))
		return sid
		
	def to_bytes(self):
		t = chr(self.Revision)
		t += chr(self.SubAuthority)
		t += struct.pack('>Q', self.IdentifierAuthority)[2:]

		for i in self.SubAuthority:
			t += struct.pack('<I', (i.bit_length() + 7) // 8)
	...
	def __str__(self):
		...
		if self.IdentifierAuthority < 2**32:
			...
			t += '0x' + struct.pack(
				'>Q', self.IdentifierAuthority)[2:].encode(
					'hex').upper().rjust(12, '0')
		for i in self.SubAuthority:
			t += '-' + str(i)
		return t
