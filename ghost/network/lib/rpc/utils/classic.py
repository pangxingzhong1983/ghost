...
from ghost.network.lib.compat import pickle, execute, is_py3k
from ghost.network.lib.rpc.core.service import ModuleNamespace
...
def upload(conn, localpath, remotepath, filter = None, ignore_invalid = False, chunk_size = 16000):
    ...
    if os.path.isdir(localpath):
        ...
        if not ignore_invalid:
            raise ValueError(f"cannot upload %r" % (localpath,))
...
def download(conn, remotepath, localpath, filter = None, ignore_invalid = False, chunk_size = 16000):
    ...
    if conn.modules.os.path.isdir(remotepath):
        ...
        if not ignore_invalid:
            raise ValueError(f"cannot download %r" % (remotepath,))
...
def teleport_function(conn, func):
    ...
    """
    from ghost.network.lib.rpc.utils.teleportation import export_function
