...
def is_proxiable(chain, transport_info):
    ...
    if not issubclass(transport_info.transport.client, GhostTCPClient):
        return False
...
def make_args_for_transport_info(transport_info, host_info, chain):
    ...
    if _is_native_for(chain, transport_info.transport):
        ...
        if client is GhostTCPClient:
            client = GhostProxifiedTCPClient
        elif client is GhostSSLClient:
            client = GhostProxifiedSSLClient
        else:
            raise ValueError(
