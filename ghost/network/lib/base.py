...
class BaseGhostTransport(object):
    __slots__ = (
...
class BaseTransport(BaseGhostTransport):
    """ obfsproxy style alias """
    __slots__ = ()
...
from .streams.GhostSocketStream import addGetPeer

from ghost.network.lib import getLogger
logger = getLogger('chain')


class TransportWrapper(BaseGhostTransport):
