...
__all__ = (
    ...
    'GhostTCPClient',
    'GhostUDPClient',
    'GhostProxifiedTCPClient',
    'GhostSSLClient',
    'GhostProxifiedSSLClient'
)
...
class GhostProxifiedTCPClient(GhostTCPClient):
    def __init__(self, *args, **kwargs):
        self.proxies = kwargs.pop('proxies', None)
        if self.proxies is None:
            raise ValueError('proxies must be specified')

        super(GhostProxifiedTCPClient, self).__init__(*args, **kwargs)
...
class GhostSSLClient(GhostTCPClient):
    def __init__(self, *args, **kwargs):
        ...
        if self.ssl_auth:
            ...
            try:
                ...
                self.SSL_CLIENT_CERT = ghost_credentials.SSL_CLIENT_CERT
                self.SSL_CLIENT_KEY = ghost_credentials.SSL_CLIENT_KEY
                self.SSL_CA_CERT = ghost_credentials.SSL_CA_CERT
                self.ROLE = 'CLIENT'
    ...
    def connect(self, host, port):
        if self.ssl_auth:
            return self.connect_ghost(host, port)
        else:
            return self.connect_any(host, port)

    def connect_any(self, host, port):
        socket = super(GhostSSLClient, self).connect(host, port)
    ...
    def connect_ghost(self, host, port):
        socket = super(GhostSSLClient, self).connect(host, port)
        try:
...
class GhostProxifiedSSLClient(GhostSSLClient, GhostProxifiedTCPClient):
    pass


class GhostUDPClient(GhostClient):
    def __init__(
        self, family=socket.AF_UNSPEC,
            socktype=socket.SOCK_DGRAM, timeout=3):
        self.sock = None
        super(GhostUDPClient, self).__init__()
        self.family = family
        self.socktype = socktype
        self.timeout = timeout
