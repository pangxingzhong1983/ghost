...
__all__ = (
    ...
    'OnlineStatusRequest', 'GhostState', 'CustomEvent',
...
from ghost.network.lib.picocmd import (
...
try:
    from ghost.network.lib import online
except ImportError:
    online = None
...
class GhostState(Command):
    ...
    def unpack(data):
        ...
        connected = records[0] & GhostState.IS_CONNECTED
        pstore_dirty = records[0] & GhostState.IS_DIRTY
        has_ipv6 = records[0] & GhostState.HAS_IPV6
        support_connect_v2 = records[0] & GhostState.SUPPORT_CONNECT_V2
        has_emergency_mode = records[0] & GhostState.HAS_EMERGENCY_MODE
    ...
    def pack(self):
        ...
        record = 0
        if self.connected:
            record |= GhostState.IS_CONNECTED

        if self.pstore_dirty:
            record |= GhostState.IS_DIRTY

        if self.has_ipv6:
            record |= GhostState.HAS_IPV6

        if self.support_connect_v2:
            record |= GhostState.SUPPORT_CONNECT_V2

        if self.has_emergency_mode:
            record |= GhostState.HAS_EMERGENCY_MODE
    ...
    def __repr__(self):
        return '{{GHOST-STATE: CONNECTED={} PSTORE={} ' \
...
class Parcel(object):
    ...
    registered_commands = EncodingTable(
        ...
        PortQuizPort, GhostState, CustomEvent,
        # V2 Commands
        SystemInfoEx, ConnectEx, RegisterHostnameId,
        DataTransferControl, DataTransferPayload, InBandExecute
    )
