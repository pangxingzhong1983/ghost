...
__all__ = ('GhostTCPServer', 'GhostUDPServer')
...
from .streams.GhostSocketStream import GhostChannel
from ghost.network.lib.connection import GhostConnection, GhostConnectionThread
from ghost.network.lib.rpc.utils.server import ThreadedServer, AuthenticationError

from ghost.network.lib.igd import UPNPError


class GhostTCPServer(ThreadedServer):
    def __init__(self, *args, **kwargs):
        ...
        self.ghost_srv = kwargs["ghost_srv"]
        ...
        if self.ghost_srv:
            try:
                ping = self.ghost_srv.config.get('ghostd', 'ping')
        ...
        if self.ping:
            ...
            if self.ghost_srv:
                self.ping_timeout = self.ghost_srv.config.get(
                    'ghostd', 'ping_interval')
            else:
...
class GhostUDPServer(object):
    ...
    def __init__(self, service, **kwargs):
        ...
        if self.ping:
            ...
            if self.ghost_srv:
                self.ping_timeout = self.ghost_srv.config.get('ghostd', 'ping_interval')
            else:
                self.ping_timeout = self.ping_interval * 10
        else:
            self.ping_interval = None
    ...
    def new(self, addr, ckcp):
        ...
        connthread = GhostConnectionThread(
            self.ghost_srv,
            self.service,
            GhostChannel(client),
