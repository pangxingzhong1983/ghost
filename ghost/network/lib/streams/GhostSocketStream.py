...
from ghost.network.lib.compat import select, select_error, get_exc_errno
from ghost.network.lib.rpc.core import SocketStream, Channel
...
class GhostChannel(Channel):
    def __init__(self, *args, **kwargs):
        super(GhostChannel, self).__init__(*args, **kwargs)
        self.compress = True
...
class GhostSocketStream(SocketStream):
    def __init__(self, sock, transport_class, transport_kwargs):
        super(GhostSocketStream, self).__init__(sock)
    ...
    def _upstream_recv(self):
        ...
        if len(self.downstream)>0:
            ...
            self.downstream.write_to(super(GhostSocketStream, self))
...
class GhostUDPSocketStream(object):
    NEW = b'\x00'
    DAT = b'\x01'
    END = b'\x02'
    ...
    def __init__(self, sock, transport_class, transport_kwargs={}, client_side=True, close_cb=None, lsi=5):
        ...
        if not (type(sock) is tuple and len(sock) in (2,3)):
            raise Exception(
                ...
                'GhostUDPSocketStream needs a reply address/port')
