...
class GhostClientCababilities(object):
...
class GhostConnection(Connection):
    ...
    def __init__(self, ghost_srv, *args, **kwargs):
        ...
        self._client_capabilities = GhostClientCababilities()
        ...
        if ghost_srv:
            self._local_root.ghost_srv = ghost_srv
        ...
        logger.debug('New GhostConnection: (%s)', self)
    ...
    #     return super(GhostConnection, self)._netref_factory(
    #         oid, clsname, modname
    #     )
    ...
    def _handle_ping(self, data):
        if data.startswith(PING_V1_CONTROL_MAGIC):
            ...
            self._dispatch_ghost_control(*payload)
            return b''
    ...
    def _init_service_with_notify(self, timeout):
        ...
        if self._local_root:
            ...
            t = Thread(
                ...
                name="GhostConnection({}) Timeout check".format(self)
            )
            t.daemon = True
            t.start()
    ...
    def _dispatch_ghost_control(self, code, *args):
        if __debug__:
            logger.debug(
                'Processing ghost brine control: args: %s', args
            )
...
class GhostConnectionThread(Thread):
    def __init__(self, *args, **kwargs):
        if __debug__:
            logger.debug('Create connection thread')

        self.ghost_srv = args[0]
        self.connection = GhostConnection(*args, **kwargs)

        Thread.__init__(self)
        self.daemon = True
        self.name = 'GhostConnection({}) Thread'.format(self.connection)
