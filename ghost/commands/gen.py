...
from ghost.ghostlib.GhostOutput import Info, Warn, Success, Error

from ghost.ghostlib.utils.listener import get_listener_ip, get_listener_port
from ghost.ghostlib.utils.listener import get_listener_ip_with_local
...
def parser(server, handler, config):
    return ghostgen.get_parser(GhostArgumentParser, config=config)
...
def do(server, handler, config, args):
    ...
    try:
        output = ghostgen.ghostgen(args, config, server, handler.display)

    except ghostgen.NoOutput:
        return
