...
class Task(Thread):
    ...
    def __init__(self, manager, *args, **kwargs):
        ...
        ghost.agent.dprint('Create task {}', self.__class__.__name__)
    ...
    def append(self, result):
        ...
        try:
            ...
            ghost.agent.remote_error('Task (append) error: {}', self.name)

    def broadcast_event(self, eventid, *args, **kwargs):
        ghost.agent.broadcast_event(eventid, *args, **kwargs)

    def stop(self):
        ghost.agent.dprint('Stopping task {}', self.__class__.__name__)

        if self._stopped and self.active:
            self._stopped.set()

    def run(self):
        ghost.agent.dprint('Task {} started', self.__class__.__name__)

        try:
            self.task()
        except:
            ghost.agent.remote_error('Task (run) error: {}', self.name)
        finally:
            ghost.agent.dprint('Task {} completed', self.__class__.__name__)
    ...
    def active(self):
        ...
        try:
            ...
            ghost.agent.remote_error('Task (active) error: {}', self.name)
            return False
    ...
    def create(self, klass, *args, **kwargs):
        ...
        if name not in self.tasks:
            try:
                ...
                ghost.agent.remote_error('Manager (create): {}', name)
    ...
    def stop(self, klass, force=False):
        ...
        if name in self.tasks:
            try:
                ...
                ghost.agent.remote_error('Manager (stop): {}', name)
                if force:
                    del self.tasks[name]
    ...
    def event(self, event):
        for task in self.tasks.values():
            try:
                ...
                ghost.agent.remote_error('Manager (event): {} evt={}', task.name, event)
