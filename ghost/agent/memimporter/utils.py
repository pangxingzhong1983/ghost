...
try:
    ...
    ghost.agent.dprint('_Py_PackageContext not found in pythonapi')
"""
...
def load_library_common(
    ...
    ghost.agent.dprint('load_library_common: Written {} to {}@{}: {} (closed={})'.format(
    ...
    ghost.agent.dprint("loading {} with init {}", module_name, initfuncname)
    ...
    try:
        ...
        ghost.agent.dprint('load_library_common: Library loaded: {}', lib)
    except Exception as e:
        ghost.agent.dprint('load_library_common: failed to load library {}: {}', name, e)
        lib = ctypes.CDLL(filepath)
        ghost.agent.dprint('load_library_common: Library loaded: {} (fallback CDLL)', lib)
    ...
    if initfunc:
        ...
        initfunc.argtypes = []
        ghost.agent.dprint('load_library_common: init found: {}', initfuncname)
        ...
        with package_context(module_name.encode('utf8')):
            ...
            ghost.agent.dprint('PyInit result: {}', m_ptr)

            #d = PyModule_GetDef(m_ptr)
            #ghost.agent.dprint('PyModule_GetDef result: {}', d)
            ...
            ghost.agent.dprint('PyModuleDef_Init called: %s'%defmod)
            ...
            if defmod:
                ...
                ghost.agent.dprint('spec: {}', spec)
                module = _imp.create_dynamic(spec)#PyModule_FromDefAndSpec(defmod, spec, 1013)
                ghost.agent.dprint('PyModule_FromDefAndSpec2 result: {}', module)
                ...
                #ghost.agent.dprint('def: {} state: {}', d, s)
                #if not s:
                #    ghost.agent.dprint("executing PyModule_ExecDef ...")
                #    PyModule_ExecDef(module, d)
                #    pass

                ghost.agent.dprint('returning module !')
                return module
            ...
            ghost.agent.dprint("result: _PyImport_FixupExtensionObject {}", res)
            if res < 0:
