...
try:
    ...
    def memfd_is_supported():
        if not sys.platform.startswith('linux'):
            ghost.agent.dprint('memfd: disabled for non-linux')
            return False

        if platform.system() == 'Java':
            ghost.agent.dprint('memfd: disabled for jython')
            return False
        ...
        if kv_maj < 3:
            ghost.agent.dprint('memfd: kernel too old (maj < 3)')
            return False

        elif kv_maj == 3 and kv_min < 13:
            ghost.agent.dprint('memfd: kernel too old (maj == 3, min < 13)')
            return False

        if NR_memfd_create is None:
            ghost.agent.dprint('memfd: Syscall NR is not defined')
            return False

        fd = _memfd_create('probe')
        if fd == -1:
            ghost.agent.dprint('memfd: probe failed')
            return False
        ...
        try:
            supported = os.path.isfile(
                ...
                'proc', 'self', 'fd', str(fd)))
            ghost.agent.dprint('memfd: supported={} (fd={})', supported, fd)
