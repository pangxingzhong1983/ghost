...
class PortFwdModule(GhostModule):
    ...
    def __init__(self, *args, **kwargs):
        GhostModule.__init__(self, *args, **kwargs)
        self.portfwd_dic={}
        self.current_id=1

    @classmethod
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog='portfwd', description=cls.__doc__)
    ...
    def run(self, args):
        if args.local:
            ...
            self.client.load_package("ghostutils.portfwd")
            remote_server = self.client.conn.modules["ghostutils.portfwd"].ThreadedRemotePortFwdServer((remote_addr, remote_port), callback=get_remote_port_fwd_cb((remote_addr, remote_port),(local_addr, local_port)))
