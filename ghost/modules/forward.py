...
from ghost.ghostlib.GhostModule import (
    GhostModule, config, GhostArgumentParser
)
...
Local port forwarding. Listen locally on 1234 and connection establishes by the target to 192.168.0.2:8000.
        ...
        actions.add_argument(
            '-VRL', '--virtual-remote-listener', help='Create virtual ghost listener',
            metavar=('PORT', 'TRANSPORT'), nargs=2
        )

        actions.add_argument(
            '-CVRL', '--cancel-virtual-remote-listener',
            metavar='PORT', help='Disable virtual ghost listener',
            type=int
        )
    ...
    def _portmap(self, state, args):
        if len(self.job.ghostmodules) > 1 and (args.local or args.cancel_local):
            raise ValueError(
        ...
        try:
            if args.virtual_remote_listener:
                ...
                self.success('Forwarding ghost port activated ({}: {})'.format(
