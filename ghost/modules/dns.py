...
from ghost.ghostlib.GhostOutput import Table, NewLine
...
@config(cat="admin")
class DNS(GhostModule):
    """ retrieve domain name from IP and vice versa """

    dependencies = ['ghostutils.dns']

    @classmethod
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog="dns", description=cls.__doc__)
        cls.arg_parser.add_argument('targets', type=str, nargs='+', help='Domain name or IP address')

    def run(self, args):
        launch_dns_ip_resolver = self.client.remote('ghostutils.dns', 'launch_dns_ip_resolver')
        launch_reverse_ip_resolver = self.client.remote('ghostutils.dns', 'launch_reverse_ip_resolver', False)
