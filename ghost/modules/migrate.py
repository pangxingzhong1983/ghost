...
from ghost.modules.lib.windows.migrate import migrate as win_migrate
from ghost.modules.lib.linux.migrate import migrate as lin_migrate
from ghost.modules.lib.linux.migrate import ld_preload

__class_name__ = 'MigrateModule'


@config(cat='manage', compat=['linux', 'windows'])
class MigrateModule(GhostModule):
    ''' Migrate ghost into another process using reflective DLL injection '''
    ...
    def init_argparse(cls):
        ...
        cls.arg_parser.add_argument(
            ...
            'until ghost exists (Linux)'
        )

        cls.arg_parser.add_argument(
            '-d', '--debug', action='store_true', default=False,
            help='enable debug option in ghost config'
        )
        ...
        cls.arg_parser.add_argument(
            ...
            'the current ghost session running'
        )
        cls.arg_parser.add_argument(
        ...
        if self.client.is_windows():
            ...
            if isBindConnection:
                ...
                self.success(
                    ...
                    "'connect --host {0}' in ghost shell".format(
