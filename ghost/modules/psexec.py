...
class PSExec(GhostModule):
    ...
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog="psexec", description=cls.__doc__)
        ...
        sgroup.add_argument(
            ...
            help='Execute a command. Use ghost64/ghost86 for .NET loader. '
    ...
    def run(self, args):
        ...
        psexec = self.client.remote('ghostutils.psexec', 'psexec', False)

        completions = []

        for host in hosts:
            if args.command in ('ghost86', 'ghost32', 'ghost64'):
                _, completion = powerloader.serve(
                    ...
                    arch='x64' if args.command == 'ghost64' else 'x86'
                )
