...
from ghost.ghostlib.GhostModule import config, GhostModule, GhostArgumentParser, IgnoreModule
...
class RFSManager(object):
    ...
    def assign(self, client):
        ...
        for oms in om_stat:
            setattr(rops, oms, client.remote('ghostutils.basic_cmds', 'd'+oms))
    ...
    def mount(self, rpath, lpath):
        ...
        self._mounts[lpath] = GhostFUSE(
            GhostRFS(rpath, self._rops), lpath,
...
class RemoteFS(GhostModule):
    ...
    def init_argparse(cls):
        parser = GhostArgumentParser(prog='rfs', description=cls.__doc__)
