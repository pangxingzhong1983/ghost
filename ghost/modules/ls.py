...
from ghost.ghostlib.GhostOutput import Color
from ghost.modules.lib import size_human_readable, file_timestamp
from ghost.ghostlib.utils.term import symbol_len
...
class ls(GhostModule):
    ...
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog="ls", description=cls.__doc__)
    ...
    def run(self, args):
        try:
            ls = self.client.remote('ghostutils.basic_cmds', 'ls')
        ...
        for r in results:
            ...
            if T_FILES in r:
                ...
                self.error('Old format. Update ghostutils.basic_cmds')
                return
