...
class apps(GhostModule):
    """ to interact manage applications """

    @classmethod
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog='apps', description=cls.__doc__)
    ...
    def run(self, args):
        ...
        self.client.load_package("ghostdroid.apps")
        if args.get_all or args.get_all_detailed:
            self.success("Getting applications installed...")
            apps = self.client.conn.modules['ghostdroid.apps'].getAllAps()
