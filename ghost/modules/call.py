...
class call(GhostModule):
    ...
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog='call', description=cls.__doc__)
    ...
    def run(self, args):
        ...
        self.client.load_package("ghostdroid.call")
        path = getLocalAndroidPath(self.client, args)
        if args.get_all:
            self.success("Getting call details...")
            callDetails = self.client.conn.modules['ghostdroid.call'].getCallDetails()
...
def getLocalAndroidPath(client, args):
    ...
    '''
    localPath = os.path.join(args.localOutputFolder, "{0}-{1}".format(client.conn.modules['ghostdroid.utils'].getAndroidID(), client.desc['user']))
