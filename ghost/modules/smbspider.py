...
from ghost.ghostlib.utils.term import colorize
import os

__class_name__="SMBSpider"

@config(cat="admin")
class SMBSpider(GhostModule):

    """ walk through a smb directory and recursively search a string into files """

    dependencies = [
        'impacket', 'calendar', 'ntpath', 'ghostutils.smbspider'
    ]
    ...
    def init_argparse(cls):
        ...
        cls.arg_parser = GhostArgumentParser(prog="smbspider", description=cls.__doc__, epilog=example)
    ...
    def run(self, args):
        ...
        smb = self.client.conn.modules["ghostutils.smbspider"].Spider(hosts, args.domain, args.port, args.user, args.passwd, args.hash, args.content, args.share, search_str, args.extensions, args.max_size, args.spider, args.depth)
