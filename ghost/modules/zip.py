...
@config(cat="admin")
class Zip(GhostModule):
    """ zip / unzip file or directory """

    dependencies = ['ghostutils.zip', 'zipfile']

    @classmethod
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog="zip", description=cls.__doc__)
    ...
    def run(self, args):
        if args.l:
            result, data = self.client.conn.modules["ghostutils.zip"].list(args.source)
            if result:
                data = obtain(data)
                log = args.source + ':\n' + '\n'.join(
            ...
            result, log = self.client.conn.modules["ghostutils.zip"].zip(args.source, args.destination)
        else:
            result, log = self.client.conn.modules["ghostutils.zip"].unzip(args.source, args.destination)
