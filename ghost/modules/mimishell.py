...
from ghost.ghostlib.GhostModule import (
    config, GhostArgumentParser,
    REQUIRE_TERMINAL
)

from ghost.modules.memory_exec import MemoryExec
...
class Mimishell(MemoryExec):
    ...
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog="mimikatz", description=cls.__doc__)
    ...
    def run(self, args):
        ...
        if not os.path.isfile(mimikatz_path):
            self.error("Mimikatz exe %s not found ! please edit Mimikatz section in ghost.conf"%mimikatz_path)
            return
