...
@config(cat="admin")
class ShellExec(GhostModule):
    """ execute shell commands on a remote system """

    dependencies = {
        'all': ['ghostutils.safepopen'],
    ...
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog='shell_exec', description=cls.__doc__)
    ...
    def run(self, args):
        if not args.hide:
            check_output = self.client.remote('ghostutils.safepopen', 'check_output', False)
