...
from ghost.network.lib.online import external_ip
...
def get_listener_port(config, external=False):
    port = None
    if external:
        port = config.get('ghostd', 'external_port')

    if not port:
        port = config.get('ghostd', 'port')
...
def get_listener_ip_with_local(cache=True, external=False, config=None, igd=None):
    ...
    if not LISTENER_IP_EXTERNAL and config:
        try:
            LISTENER_IP_EXTERNAL = config.getip('ghostd', 'external')
        except AddrFormatError:
            LISTENER_IP_EXTERNAL = None

        if not LISTENER_IP_EXTERNAL and config.getboolean(
            'ghostd', 'allow_requests_to_external_services'
        ):
            ...
            if not LISTENER_IP_EXTERNAL:
                ipv6 = config.getboolean('ghostd', 'ipv6')
    ...
    if not LISTENER_IP_LOCAL and config:
        LISTENER_IP_LOCAL = config.getip('ghostd', 'address')
