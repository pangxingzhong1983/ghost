...
from ghost.ghostlib.utils.arch import is_native, same_as_local_arch
from ghost.ghostlib import ROOT, getLogger
...
WELL_KNOWN_DEPS = {
    ...
    'ghostutils.basic_cmds': {
        'windows': ['junctions', 'ntfs_streams'],
        'linux': ['xattr'],
        'all': [
            'ghostutils', 'scandir', 'zipfile',
...
def bootstrap(stdlib, config, autostart=True, purepy=False):
    if "ghost/agent/__init__.pyo" in stdlib:
        actions = [
            'from __future__ import absolute_import',
            'from __future__ import division',
            'from __future__ import print_function',
        ...
        actions += [
            ...
            'spec = importlib.util.spec_from_loader("ghost.agent", loader=None)',
            'agent = importlib.util.module_from_spec(spec)',
            'agent.__file__ = str("ghost://ghost/agent/__init__.pyo")',
            'agent.__package__ = str("ghost.agent")',
            'agent.__path__ = [str("ghost://ghost/agent/")]',

            'exec(marshal.loads(stdlib["ghost/agent/__init__.pyo"][8:]), agent.__dict__)',
        ]
        ...
        actions +=[
            ...
            'spec = importlib.util.spec_from_loader("ghost.agent", loader=None)',
            'agent = importlib.util.module_from_spec(spec)',
            'agent.__file__ = str("ghost://ghost/agent/__init__.py")',
            'agent.__package__ = str("ghost.agent")',
            'agent.__path__ = [str("ghost://ghost/agent/")]',

            'exec(stdlib["ghost/agent/__init__.py"].decode("utf8"), agent.__dict__)',
        ]
...
def importer(
    ...
    if as_bundle or as_dict:
        ...
        return 'ghostimporter.ghost_add_package({}, compressed=True)'.format(
            reprb(blob))
        ...
        for i in range(1, len(tab)-1):
            ...
            if not target.purepy:
                ...
                if f not in modules and f not in toadd and f[:-1] not in modules:
                    ...
                    toadd[f] = ghostcompile("", pathname, target=target.pyver)
            else:
