...
from ghost.ghostlib.GhostOutput import Success, Error
...
class rubber_ducky():
    ...
    def __init__(self, display, conf, ghost_conf, link_port=8080, targetOs='Windows'):
        '''
        '''
        self.conf = conf
    ...
    def __loadRubberDuckyConf__(self):
        ...
        self.encoderPath = self.ghost_conf.get('rubber_ducky', 'encoder_path')
        self.keyboardLayoutPath = self.ghost_conf.get('rubber_ducky', 'default_keyboard_layout_path')

        if self.encoderPath == 'TO_FILL':
            self.unconfigured = True
            self.display(Error('The "encoder_path" value has to be filled in ghost.conf for generating inject.bin'))

        if self.keyboardLayoutPath == 'TO_FILL':
            self.unconfigured = True
            self.display(Error(
                'The "default_keyboard_layout_path" value has to be filled in ghost.conf '
