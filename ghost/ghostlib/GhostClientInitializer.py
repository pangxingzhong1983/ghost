...
def get_uuid():
    ...
    try:
        if hasattr(ghost, 'cid'):
            cid = ghost.cid
        elif hasattr(ghost, 'client'):
            cid = ghost.client.cid
    except:
        cid = None

    proxy = None
    try:
        from ghost.network.lib.proxies import LAST_PROXY, has_wpad
        if hasattr(ghost, 'client') and ghost.client.connection_info.get(
                'proxies', []):
            try:
                proxy = ' -> '.join(
                    '{}://{}{}'.format(
    ...
    try:
        ...
        from ghost.network.lib.online import LAST_EXTERNAL_IP
        if LAST_EXTERNAL_IP:
