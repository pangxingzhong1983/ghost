...
from ghost.network.lib import socks
...
class GhostOffloadDNS(threading.Thread):
    QTYPE = QTYPE.reverse
    ...
    def _serve(self):
        ...
        while self.active:
            ...
            qtype = GhostOffloadDNS.QTYPE[qtype]
...
class GhostOffloadSocket(object):
...
class GhostOffloadAcceptor(object):
...
class GhostOffloadManager(object):
    ...
    def dns(self, handler, domain):
        return GhostOffloadDNS(self, handler, domain)

    def tcp(self, port, extra={}):
        return GhostOffloadAcceptor(self, REQUEST_TCP_LISTENER, port, extra)

    def kcp(self, port, extra={}):
        return GhostOffloadAcceptor(
            self, REQUEST_KCP_LISTENER, port, extra, KCP_MTU
        )

    def ssl(self, port, extra={}):
        return GhostOffloadAcceptor(self, REQUEST_TLS_LISTENER, port, extra)
