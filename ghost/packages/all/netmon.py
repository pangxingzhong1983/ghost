...
def netmon_start(event_id=None):
    if ghost.agent.manager.active(NetMon):
        return False

    ghost.agent.manager.create(NetMon, event_id=event_id)
    return True


def netmon_dump():
    netmon = ghost.agent.manager.get(NetMon)
...
def netmon_stop():
    netmon = ghost.agent.manager.get(NetMon)
    if netmon:
        ghost.agent.manager.stop(NetMon)
