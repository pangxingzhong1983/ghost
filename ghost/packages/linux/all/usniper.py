...
def start(path, addr, reg='ax', ret=False, cast=None, argtype='chr', event_id=None):
    try:
        if ghost.agent.manager.active(USniper):
            return False
    except:
        try:
            ghost.agent.manager.stop(USniper)
        except:
            pass
    ...
    return ghost.agent.manager.create(
...
def stop():
    return ghost.agent.manager.stop(USniper)


def dump():
    usniper = ghost.agent.manager.get(USniper)
    if usniper:
