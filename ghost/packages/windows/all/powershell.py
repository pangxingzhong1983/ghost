...
def loaded(name=None):
    powershell = ghost.agent.manager.get(PowerHost)
    if not powershell:
        if not name:
            return []
        else:
...
def load(name, content, force=False, try_x64=False, daemon=False, width=None, v2=None):
    powershell = ghost.agent.manager.get(PowerHost) or \
      ghost.agent.manager.create(PowerHost)
...
def stop():
    ghost.agent.manager.stop(PowerHost)
