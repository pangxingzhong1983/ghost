...
static const char module_doc[] = DOC("Builtins utilities for ghost");
...
const char *__pathmap_callback(const char *path, char *buf, size_t buf_size)
{
    ...
    if (!strncmp(path, "f:", 2) ||
        !strncmp(path, "ghost:/", 6) ||
        !strncmp(path, "ghost/", 5))
    {
        dprint("__pathmap_callback(%s) -> ghost -> NULL\n");
        return NULL;
    }
...
{
    return PyBool_FromLong(ghost_memfd_supported());
}
...
static PyObject *Py_memfd_create(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    fd = ghost_memfd_create(memfd_path, sizeof(memfd_path));
...
static PyMethodDef methods[] = {
    ...
    { "get_arch", Py_get_arch, METH_NOARGS, DOC("get current ghost architecture (x86 or x64)") },
#ifdef Linux
#ifdef _FEATURE_INJECTOR
...
static struct PyModuleDef GhostModuleDef =
{
...
FUNC_EXPORT PyInit__ghost(void) {

    PyObject *ghost;
    //PyObject *ghost = Py_InitModule3("_ghost", methods, (char *) module_doc);
    dprint("creating ghost module ...\n");
    ghost = PyModule_Create(&GhostModuleDef);

    if (!ghost) {
        return NULL;
    }

    PyModule_AddStringConstant(ghost, "revision", GIT_REVISION_HEAD);
    ExecError = PyErr_NewException("_ghost.error", NULL, NULL);
    Py_INCREF(ExecError);
    PyModule_AddObject(ghost, "error", ExecError);
    ...
    PyModule_AddObject(ghost, "pathmap", py_pathmap);
#ifndef _LD_HOOKS_NAME
