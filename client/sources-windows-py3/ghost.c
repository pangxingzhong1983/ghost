...
static char module_doc[] = DOC("Builtins utilities for ghost");
...
void on_exit_session(void) {
    ...
    dprint(
        "ghost:on_exit_session called, current callback: %p\n",
...
FUNC_EXPORT PyInit__ghost(void) {
        PyObject *ghost;
        //PyObject *ghost = Py_InitModule3("_ghost", methods, module_doc);
        ...
        if (PyModule_AddObject(ghost, "error", ExecError) < 0) {
...
BOOL WINAPI DllMain( HINSTANCE hinstDLL, DWORD dwReason, LPVOID lpReserved )
{
    ...
    dprint("Call DllMain (_ghost) %d/%p\n", dwReason, lpReserved);
    ...
    switch( dwReason )
    {
        ...
        case DLL_PROCESS_ATTACH:
            ...
            if (lpReserved > 0xFFFF) {
                _ghost_pyd_args_t *args =
                        (_ghost_pyd_args_t*) lpReserved;
