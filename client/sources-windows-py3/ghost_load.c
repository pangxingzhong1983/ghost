...
#ifdef DEBUG
#include "_ghost_debug_pyd.c"
#define _ghost_pyd_c_start _ghost_debug_pyd_c_start
#define _ghost_pyd_c_size _ghost_debug_pyd_c_size
#else
#include "_ghost_pyd.c"
#endif
#endif
...
void initialize(BOOL isDll) {
    ...
    */
    ...
    dprint("Load _ghost\n");
    HMODULE ghosthMod = xz_dynload(
        "_ghost.pyd",
        _ghost_pyd_c_start, _ghost_pyd_c_size,
        &args
    );

    if (args.blInitialized != TRUE) {
        dprint("_ghost.pyd initialization failed\n");
        return;
    }

    typedef FARPROC (*PyInit__ghostT)(void);
    PyInit__ghostT PyInit__ghost;
    PyInit__ghost = (PyInit__ghostT)MyGetProcAddress(ghosthMod, "PyInit__ghost");
...
DWORD WINAPI _run_ghost_thread(LPVOID lpArg)
{
#ifdef POSTMORTEM
    __try {
        dprint("Ghost worker started [Postmortem enabled]\n");
        run_ghost();
    }
...
#else
    dprint("Ghost worker started [Postmortem disabled]\n");
    run_ghost();
#endif
    dprint("Ghost worker exited\n");
    return 0;
}
